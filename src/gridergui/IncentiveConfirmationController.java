...
//    INCENTIVES TABLE
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        if (!oApp.isMainOffice()) {
            ...
            if (oApp.getDepartment().equals(COLLECTION)) {
                if (oApp.getEmployeeNo().equals("M00109011794")) {//if user is  maam loraine
                    oTrans.setTranStat(1);
                } else {
                    oTrans.setTranStat(0);
                }

            } else if (oApp.getDepartment().equals(AUDITOR)) {
                oTrans.setTranStat(1);

            } else {
                oTrans.setTranStat(0);//sales in M001 
            }
        }
